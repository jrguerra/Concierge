...
class EventsDatabase extends Database {
	...
	public static function initialize() {
		// this should be called only once, or else it will fail in the subsequent ones
...
class ItineraryDatabase extends Database {
	...
	public static function addEventToItinerary($eventId, $day, $time) {
	...
	public static function formatted_itinerary() {
		$result = parent::getRows(self::$db, self::$tableName);
		
		$arr;
		while ($row = mysql_fetch_array($result)) {
			$arr[$row['Time']] = $row['eventID'];
		}
		
		return $retVal;
		
		// for loop
		// format the array here! TODO!
	...
	public static function getItinerary() {
		...
		$retVal;
		while ($row = mysql_fetch_array($result)) {
			$retVal[$row['Time']] = $row['eventID'];
		}
		
		return $retVal;
	}
	
	public static function setEventForTimes($eventId, $time1, $time2) {
		for ($i = $time1; $i <= $time2; $i ++) {
